...

{
  node(label: 'io') {
    ...
    previous_build_release_value = item.getLastCompletedBuild().getEnvironment(null).get('release')
    echo "${previous_build_result}"
    echo "${previous_build_release_value}"
    }
    container('docker') {
      docker.withRegistry(docker_repo_url, "ecr:${aws_region}:${docker_repo_creds_id}") {
        ...
        if ("${release}" == "major"){
          ...
          current_buildnumber = get_build_number("${release}", "${previous_build_release_value}")
...
def get_build_number(release, previous_build_release_value) {
  
  if ((release == previous_build_release_value) && previous_build_result == "FAILURE"){
